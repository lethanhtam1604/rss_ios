...
class AddStaffViewController: UIViewController, UITextFieldDelegate, LocationDelegate {
    ...
    override func viewDidLoad() {
        ...
        staffFirstNameField.returnKeyType = .next
        ...
        staffLastNameField.returnKeyType = .next
    ...
    func save() {
        ...
        let password = UserDefaultManager.getInstance().getCurrentPasswordUser()
        let uid = (FIRAuth.auth()!.currentUser?.uid)!
        
        if !isEdit {
            FIRAuth.auth()!.createUser(withEmail: self.emailField.text!, password: "123456") { user, error in
                if error == nil {
                    FIRAuth.auth()?.sendPasswordReset(withEmail: self.emailField.text!) { error in
                        if error == nil {
                            self.staff.id = (user?.uid)!
                            ...
                            FIRAuth.auth()!.signIn(withEmail: email!, password: password!, completion: {(user: FIRUser?, error) in
                                DatabaseHelper.shared.saveUser(user: self.staff) {
                                    ...
                                    Utils.showAlert(title: "Successfully", message: "Your new password has been sent to " + self.emailField.text! + ". Please check your email!", viewController: self)
                                }
                            })
                        }
                        else {
